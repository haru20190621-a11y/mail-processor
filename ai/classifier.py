...
カテゴリ:
- urgent   : 重要・緊急（以下のいずれかに該当するもの）
              ・クレーム、苦情、修正依頼、納品物への指摘
              ・発注依頼、仕事の依頼、見積もり依頼（新規ビジネスの可能性）
              ・法的通知、督促、契約に関する重要連絡
              ・緊急の取引先・顧客からの連絡
              ※ビジネス上の対応が必要なメールは緊急度に関わらず urgent にする
- reply    : 返信必要（返信は必要だがビジネス上の緊急性がないもの。一般的な質問、提案など）
...
- reply: 取引先・顧客・仕事関係者からの返信が必要なメールは true。一方的な営業・宣伝・マーケティングメールの「ご確認ください」は false
- fyi / sales: 常に false
